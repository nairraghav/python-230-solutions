...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(room_number):
        ...
        try:
            return ['The room is covered with puke yellow wallpaper',
                    'Blue wallpaper covers the ceiling while the floor is '
                    'strewn with green',
                    'Red flakes are peppered all over the ground, the room is '
                    'extremely warm',
                    'The room is pitch black, nothing can be made out'
                    ][room_number]
        except IndexError:
    ...
    def get_input(self):
        ...
        full_message = b''
        partial_message = b''
        while not partial_message.endswith(b'\n'):
            partial_message = self.client_connection.recv(16)
            full_message += partial_message

        self.input_buffer = full_message.decode().strip()
    ...
    def move(self, argument):
        ...
        * "west"
        ...
        if (
                self.room == 1 and argument == 'east'
        ) or (
                self.room == 2 and argument == 'west'
        ) or (
                self.room == 3 and argument == 'south'
        ):
            self.room = 0
